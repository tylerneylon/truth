...
function orderElts(xys) {
    ...
    // 5. Render all "away" faces, and then all "toward-us" faces.
    for (let polygon of ctx.facePolygons) {
        if (polygon.isFacingAway) mainGroup.appendChild(polygon);
    }
    for (let polygon of ctx.facePolygons) {
        if (!polygon.isFacingAway) mainGroup.appendChild(polygon);
    }
...
export function updatePoints() {
    ...
    for (let line of ctx.lines) {
        ...
        if (ctx.fadeRange) {
            ...
            // XXX
            line.elt.setAttribute('display', 'none');
        }
