...
\begin{obs}
    Ideas are neither true nor false, but have degrees of truthfulness.
    \begin{itemize}
...
If I can succeed in achieving my own later goals by making using of an
...
balls are lucky. If I only have a red bowling ball and an orange one,
and there's
something wrong with the blue one, then my belief that red bowling balls are
...
means, ``I usually get a higher score when I use one versus the other.'' If we
tried to apply that same idea to many other bowling balls, then --- just as
Newtonian mechanics needs to be qualified --- we would either have to discard or
idea completely, or add a restriction to it. In other words, if you believe
the laws of Newtonian mechanics have some truth, then you must likewise accept
other approximations for reality as equally true.
...
Here's a high-level argument that effective truth really does match our
intuition for truth:
\begin{itemize}
    ...
    \item Suppose an idea will pass every test we ever give it. Then the idea
    ...
    \item Suppose there is some test which the idea would fail.
...
% TODO HERE
%
% I think reframe as two objection-refutations:
% (a) sky-is-blue has no goal
% (b) red-bowling-balls-are-lucky is accidentally correct
%
% Summarize as:
% * a clear failure case => not effectively true (nor intuitively true)
% * always works => intuitively true (and effectively true)
% How can we better understand what feels like bad reasoning? (ask in the
% article)
% The answer will be the limon argument. Where does this go in the paper?
%  I think it goes in the section on ignorance and goals.
%  XXX Add that note to the outline. Ideally I'll have a qX tag to refer back
%  to.
%
