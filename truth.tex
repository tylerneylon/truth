...
What distinguishes notions like species and vegetables from things like
{\em positive integers}, which seem to be more clearly-defined?
I think the key is that our intuition for a species is based on looking at
examples. It's true that scientists can walk away from the original intuition of
a notion by
changing the definition. But I'm going to argue that all of our
...
\begin{argt}[That example-based mental words are fuzzy]
    ...
    \begin{itemize}
        ...
        \item Suppose that mental word $x$ is learned by example and is
            ...
            For
            example, people learn to distinguish dogs from cats by seeing
            example images or meeting dogs and cats in person.
        ...
        \item Perhaps we've never seen such an example, but it's logically
            ...
            of $p-$values seen on $x$ examples, and similarly for $p_y$.

            Perhaps all the $p_x$ values are less than all the $p_y$ values.
            Then there must be some non-empty interval $I=(max(p_x), min(p_y))$.
            ...
            other readers. I'm appealing to the intuition that even in cases
            where we have a simple model and consistent data, we typically do
            not have a {\em uniquely-determined} hypothesis. For example, if
            $p_x=\{1.3, 2.4, 3.7\}$ and $p_y=\{4.1, 5.8\}$, and I see $p=3.9$,
            then I have no way to tell if the object is an $x$ or a $y$.}
            And this zone of ambiguity must exist for all real-valued
            properties of the object (by the same argument).
