...
conscious question is motivated by a goal. Putting these two together, every
internal idea is connected to a goal.

This is a powerful claim. One key implication is that we cannot even think of
internal ideas (eg, excluding perceptions) that are not somehow associated with
a goal. It seems possible that the goals that first led us to an idea are
different from the goals we later associated with it. For example, we may first
think of a water heater as a way to make tea, and later learn how to make
instant ramen with that same water heater.

It's also interesting to see an example of an idea we {\em cannot} think of.
If you speak only English,
then it's hard to learn to speak Mandarin because
Mandarin makes use of {\em tones}, which are a musically-oriented aspect of how
a word is pronounced. In English, if you change the pitch of your voice as you
say a word, it might hint a mood or suggest your confidence level, but it never
fundamentally changes the meaning of the word. No matter how I change the pitch
of my voice when I say {\em cookie}, you know I'm talking about a cookie. But in
Mandarin, I can say the same phonetic sounds --- the same sequence of consonants
and vowels -- in two different pitch patterns, and it indicates a completely
different meaning.
It's hard for English-speakers to learn this new aspect of Mandarin because it's
a concept that they've never had a purpose for. In other words, the
language-listening part of your brain is trained to hear consonants and vowels,
and to basically ignore everything else. Now you have to add in some new mental
vocabulary to learn Mandarin.

This isn't a perfect example of an idea we can't think of --- because we {\em
can} think of it; it's just tricky to learn for some people. It's not my fault I
gave a partial example. It's literally impossible for me to express an idea that
cannot be thought of. But I claim they exist, because we humans will never
have experienced all possible goals. For every goal we never think of,
there is the question of how to achieve that goal, and an answer to this
question. We will never think of any of these.

If it helps your intuition, consider an alien race that sees color in a
different band of the EM spectrum. Then they will have many words for ``colors''
that we would not recognize as colors. Again, technically, this example is
partial because scientists can understand what I mean --- and, again, any
...
perception, then you can somewhat imagine ideas that we will never think of.
...
We have the rough workings of an abstract model of thought. I'll add one more
ingredient to this before moving on. Earlier, I said that this article excludes
the color green from our (adjusted) meaning of an {\em idea}. 
In this section I'll start to use the term {\em mental word} to talk about
the mental pieces of ideas we can take apart and put back together again to get
new ideas. Mental words include things like
colors or water heaters or letters of the alphabet.
Intuitively, you might want to think of
ideas as {\em mental sentences}, and then mental words are whatever building
blocks these are made from.
As with the term {\em idea}, I won't try to define {\em mental word}.
Instead, I'll notice something about our intuition for mental words.

Where do mental words come from? Unlike internal ideas, it seems like external
ideas can play a role here. For example, we cannot learn the sound of a
ambulance siren
without perception.
At the same time, you probably know to drive out of the way of an ambulance when
you hear its siren. There is a goal associated with the mental word of sirens,
which might be to obey traffic laws.
...
\begin{tikzpicture}[node distance = 2cm, thick]%
    ...
    \draw[<-] (4) to node {d} (1);
    \draw[->] (1, -1) -- (1, -5) node[pos=1.0, right] {abc};
    %\draw[<-] (4) to [bend up] node [midway, left]{d} (1);
\end{tikzpicture}%
\end{center}

Just as internal thoughts are conscious actions we take, so is the formation of
a new mental word. This means that mental words, like internal ideas, can only
exist in correspondence to some goal. An example: Suppose one day you're stung
by a distinctive-looking wasp. A year later, you see a similar wasp, and you
steer clear of it, remembering the sting. There is a goal in remembering this
kind of wasp, which is not get stung. Contrast this to another bug, suppose a
quiet and unobtrusive bug that you occasionally see, but never care about at
all. If you never care about this other bug, then you don't have a mental word
for it.
...
% HERE
%
% Insert somewhere here the little diagram I have on my remarkable.
